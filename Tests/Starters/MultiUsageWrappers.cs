...
public class MultiUsageWrappers
{
    ...
    public void MultiListWrapper()
    {
        ...
        var query = data.OrderBy(a => a.Item1).Select(a => a.Item2);
        var l1 = new (int, string)[] { (3, "a"), (1, "b"), (2, "c") };
        var l2 = new List<(int, string)> { (4, "a"), (5, "b"), (6, "c") };
        data.Data = l1;
        TestUtils.EqualSequences(query, new[] { "b", "c", "a" });
        data.Data = l2;
        TestUtils.EqualSequences(query, new[] { "a", "b", "c" });
    }
    ...
    [Fact]
    public void MultiDictionaryWrapper()
    {
        var data = new MultiDictionaryWrapper<int, string>();
        var query = data.OrderBy(a => a.Key).Select(a => a.Value);
        var l1 = new Dictionary<int, string> { { 3, "a" }, { 1, "b" }, { 2, "c" } };
        var l2 = new Dictionary<int, string> { { 4, "a" }, { 5, "b" }, { 6, "c" } };
        data.Data = l1;
        TestUtils.EqualSequences(query, new[] { "b", "c", "a" });
        data.Data = l2;
        TestUtils.EqualSequences(query, new[] { "a", "b", "c" });
